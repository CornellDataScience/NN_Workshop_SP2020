...
class AsyncPredictor:
    ...
    class _PredictWorker(mp.Process):
        def __init__(self, model_path, model_class, task_queue, result_queue):
    ...
    def __init__(self, model_path, model_class, num_cpus=1, queue_size=3, ordered=True):
        ...
        assert num_cpus > 0, "Number of cpus must be specified"
        num_procs = num_cpus
        ...
        if num_cpus > 0:
            ...
            for _ in range(num_cpus):
                self.procs.append(
                    AsyncPredictor._PredictWorker(model_path, model_class, self.task_queue, self.result_queue)
                )
